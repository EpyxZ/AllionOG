...
const string strMessageMagic = "AllionOG Signed Message:\n";
...
{
    const int nMinSubsidy = 0.101 * COIN;
	int64_t nSubsidy = 0 * COIN;
    if(nHeight == 1) {
    nSubsidy = 10000000 * COIN; // 10 mil premine for payouts & services rendered (blocks 2-4 .1 ALLOG)
    } else if(nHeight >= 4 && nHeight <= 10004){ 
    nSubsidy = 5000 * COIN;    //  			5000 x 10,000 = 50 mil PoW
    } else if(nHeight >= 10005 && nHeight <= 20004){
    nSubsidy = 500 * COIN;    // 			500 x 10,000 = 5 mil PoW
    } else if(nHeight >= 20005 && nHeight <= 40004){
    nSubsidy = 1000 * COIN;     // 			1000 x 20,000 = 20 mil PoW
    ...
    nSubsidy = 500 * COIN; // 				500 x 10,000 = 5 mil PoW
    } else if(nHeight >= 80005 && nHeight <= 100004){
    nSubsidy = 1000 * COIN; // 				1000 x 20,000 = 20 mil PoW
    } else if(nHeight >= 100005 && nHeight <= 110004){
    nSubsidy = 2000 * COIN; // 				2000 x 10,000 = 20 mil PoW
    }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in AllionOG
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AllionOG", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Top US intelligence official: I submitted my resignation - CNN.COM allion.us";
        CTransaction txNew;
        ...
        //// debug print        
        block.print();
        assert(block.hashMerkleRoot == uint256("0xef1e17193a82f19ab84771d58c326450952ff373252b010c9e79dd9a9ab51d72"));
