...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AllionOG
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AllionOG
    // Mac: ~/Library/Application Support/AllionOG
    // Unix: ~/.allionog
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AllionOG";
#else
    fs::path pathRet;
...
#endif
    pConf << "#Head to allionog.life for more information\nrpcuser=AllionOG\nrpcpassword="
            + randomStrGen(15)
            + "\nrpcport=7522"
            + "\ndaemon=1"
            + "\nserver=1"
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "allionog.conf"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong AllionOG will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("AllionOG"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
